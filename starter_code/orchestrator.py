...
def main():
    ...
    def _add_document(doc):
        if not doc:
            return
        if run_quality_gate(doc):
            final_kb.append(doc)

    def _add_documents(items):
        if not items:
            return
        for item in items:
            _add_document(item)

    _add_document(extract_pdf_data(pdf_path))
    _add_document(clean_transcript(trans_path))
    _add_documents(parse_html_catalog(html_path))
    _add_documents(process_sales_csv(csv_path))
    _add_document(extract_logic_from_code(code_path))
